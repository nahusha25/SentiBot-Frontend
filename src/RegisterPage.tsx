  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/register", {
        ...
        email,
        password,
        date_of_birth: dateOfBirth, // BACKEND FORMAT
        city,
        skills,
        experience,
        career_goal: careerGoal, // BACKEND FORMAT
        qualification
      });
      ...
      <div style={{
        ...
        <form onSubmit={handleRegister} style={{ display: "grid", gap: "12px" }}>
          ...
          <select value={qualification}
            onChange={(e) => setQualification(e.target.value)} style={inputStyle}>
          ...
          <input placeholder="Skills (React, Java, UI, etc)" value={skills}
            onChange={(e) => setSkills(e.target.value)} style={inputStyle} />

          <input placeholder="Experience (Fresher, 1 year)" value={experience}
            onChange={(e) => setExperience(e.target.value)} style={inputStyle} />

          <textarea placeholder="Career Goal" value={careerGoal}
